...
const float maxSpeedJump = 40;  // Largest increment that the current speed can change per unit time on either of the two motors
const float turnRate = 0.5;
const float diagRate = 0.50; // Variable used to set the turning radius of diagonal turns
...
void setup() {
  ...
  m.setM2Speed(0);  // Set motor 1 to full coast(zero speed) when no switch is pressed
...
void checkSwitches(){
  ...
  {
    targetSpeed1 = -1 * maxSpeed * turnRate;
    targetSpeed2 = maxSpeed * turnRate;
    Serial.println("left");
  }
    ...
    {
      targetSpeed1 = maxSpeed * turnRate;
      targetSpeed2 = -1 * maxSpeed * turnRate;
